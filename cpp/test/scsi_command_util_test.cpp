...
TEST(ScsiCommandUtilTest, ModeSelect6)
{
    ...
    buf[10] = 0x02;
    buf[11] = 0x00;
    EXPECT_THAT([&] {ModeSelect(scsi_command::cmd_mode_select6, cdb, buf, LENGTH, 256);},
    ...
    buf[24] = 0x02;
    EXPECT_THAT([&] {ModeSelect(scsi_command::cmd_mode_select6, cdb, buf, LENGTH - 1, 512);},
        Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::illegal_request),
                Property(&scsi_exception::get_asc, asc::invalid_field_in_parameter_list))))
        << "Not enough command parameters";

    EXPECT_FALSE(ModeSelect(scsi_command::cmd_mode_select6, cdb, buf, LENGTH, 512).empty());
}
...
TEST(ScsiCommandUtilTest, ModeSelect10)
{
    ...
    buf[14] = 0x02;
    buf[15] = 0x00;
    EXPECT_THAT([&] {ModeSelect(scsi_command::cmd_mode_select10, cdb, buf, LENGTH, 256);},
        Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::illegal_request),
                Property(&scsi_exception::get_asc, asc::invalid_field_in_parameter_list))))
        << "Requested sector size does not match current sector size";
    ...
    // Match the requested to the current sector size
    buf[28] = 0x02;
    EXPECT_THAT([&] {ModeSelect(scsi_command::cmd_mode_select10, cdb, buf, LENGTH - 1, 512);},
        Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::illegal_request),
                Property(&scsi_exception::get_asc, asc::invalid_field_in_parameter_list))))
        << "Not enough command parameters";

    EXPECT_FALSE(ModeSelect(scsi_command::cmd_mode_select10, cdb, buf, LENGTH, 512).empty());
}
