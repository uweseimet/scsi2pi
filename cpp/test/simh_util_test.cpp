...
TEST(SimhUtilTest, ReadHeader)
{
    ...
    int64_t position = WriteHeader(file, FILE_SIZE, { simh_class::tape_mark_good_data_record, 0 });
    position += WriteHeader(file, FILE_SIZE, { simh_class::tape_mark_good_data_record, 0x1234567 });
    WriteHeader(file, FILE_SIZE, { simh_class::reserved_marker, 0 });
    ...
    EXPECT_EQ(0, ReadHeader(file, FILE_SIZE, header));
    EXPECT_EQ(header.cls, simh_class::reserved_marker);
...
TEST(SimhUtilTest, WriteHeader)
{
    ...
    int64_t position = WriteHeader(file, FILE_SIZE, { simh_class::tape_mark_good_data_record, 1 });
    EXPECT_EQ(HEADER_SIZE, position);

    position += WriteHeader(file, FILE_SIZE, { simh_class::reserved_marker, 0 });
    EXPECT_EQ(2 * HEADER_SIZE, position);

    EXPECT_EQ(-1, WriteHeader(file, FILE_SIZE, { simh_class::tape_mark_good_data_record, 0 }));
}
...
TEST(SimhUtilTest, ReadRecord)
{
    ...
    WriteRecord(file, buf.size() + HEADER_SIZE, buf, static_cast<int>(buf.size()));
    ranges::fill(buf, 0);
    file.seekg(0, ios::beg);
    EXPECT_EQ(buf.size(), ReadRecord(file, buf, static_cast<int>(buf.size())));
    ...
    // TODO Enable
    //EXPECT_EQ(-1, ReadRecord(file, buf, static_cast<int>(buf.size())));
}
...
TEST(SimhUtilTest, MoveBack)
{
    ...
    int64_t position = WriteHeader(file, FILE_SIZE, { simh_class::tape_mark_good_data_record, RECORD_LENGTH_ODD });
    position += WriteRecord(file, FILE_SIZE, buf, RECORD_LENGTH_ODD);
    position += WriteHeader(file, FILE_SIZE, { simh_class::tape_mark_good_data_record, RECORD_LENGTH_EVEN });
    position += WriteRecord(file, FILE_SIZE, buf, RECORD_LENGTH_EVEN);
    position += WriteHeader(file, FILE_SIZE, { simh_class::reserved_marker, static_cast<int>(simh_marker::erase_gap) });
    position += WriteHeader(file, FILE_SIZE,
        { simh_class::reserved_marker, static_cast<int>(simh_marker::end_of_medium) });
