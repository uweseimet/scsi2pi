...
TEST(DiskTest, Rezero)
{
    ...
    EXPECT_THAT([&] {d->Dispatch(scsi_command::cmd_rezero);}, Throws<scsi_exception>(AllOf(
    ...
    EXPECT_CALL(*controller, Status);
    disk->Dispatch(scsi_command::cmd_rezero);
    EXPECT_EQ(status::good, controller->GetStatus());
}
...
TEST(DiskTest, FormatUnit)
{
    ...
    EXPECT_THAT([&] {d->Dispatch(scsi_command::cmd_format_unit);}, Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::not_ready),
                Property(&scsi_exception::get_asc, asc::medium_not_present))))
    ...
    EXPECT_CALL(*controller, Status);
    disk->Dispatch(scsi_command::cmd_format_unit);
    EXPECT_EQ(status::good, controller->GetStatus());
    ...
    EXPECT_THAT([&]
        {
            d->Dispatch(scsi_command::cmd_format_unit)
            ;
...
TEST(DiskTest, ReassignBlocks)
{
    ...
    EXPECT_THAT([&] {d->Dispatch(scsi_command::cmd_reassign_blocks);}, Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::not_ready),
                Property(&scsi_exception::get_asc, asc::medium_not_present))))
    ...
    EXPECT_CALL(*controller, Status);
    disk->Dispatch(scsi_command::cmd_reassign_blocks);
    EXPECT_EQ(status::good, controller->GetStatus());
}
...
TEST(DiskTest, Seek6)
{
    ...
    EXPECT_THAT([&] {d->Dispatch(scsi_command::cmd_seek6);}, Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::illegal_request),
    ...
    EXPECT_CALL(*controller, Status);
    disk->Dispatch(scsi_command::cmd_seek6);
    EXPECT_EQ(status::good, controller->GetStatus());
}
...
TEST(DiskTest, Seek10)
{
    ...
    EXPECT_THAT([&] {d->Dispatch(scsi_command::cmd_seek10);}, Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::illegal_request),
                Property(&scsi_exception::get_asc, asc::lba_out_of_range))))
    ...
    EXPECT_CALL(*controller, Status);
    disk->Dispatch(scsi_command::cmd_seek10);
    EXPECT_EQ(status::good, controller->GetStatus());
}
...
TEST(DiskTest, ReadCapacity10)
{
    ...
    EXPECT_THAT([&] {d->Dispatch(scsi_command::cmd_read_capacity10);}, Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::not_ready),
                Property(&scsi_exception::get_asc, asc::medium_not_present))))
    ...
    disk->Dispatch(scsi_command::cmd_read_capacity10);
...
TEST(DiskTest, ReadCapacity16)
{
    ...
    EXPECT_THAT([&] {d->Dispatch(scsi_command::cmd_read_capacity16_read_long16);}, Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::illegal_request),
                Property(&scsi_exception::get_asc, asc::invalid_field_in_cdb))))
    ...
    EXPECT_CALL(*controller, DataIn);
    disk->Dispatch(scsi_command::cmd_read_capacity16_read_long16);
...
TEST(DiskTest, Read10)
{
    ...
    EXPECT_THAT([&] {d->Dispatch(scsi_command::cmd_read10);}, Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::illegal_request),
                Property(&scsi_exception::get_asc, asc::lba_out_of_range))))
    ...
    disk->SetBlockCount(1);
    EXPECT_CALL(*controller, Status);
    disk->Dispatch(scsi_command::cmd_read10);
    EXPECT_EQ(status::good, controller->GetStatus());
...
TEST(DiskTest, Read16)
{
    ...
    EXPECT_THAT([&] {d->Dispatch(scsi_command::cmd_read16);}, Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::illegal_request),
                Property(&scsi_exception::get_asc, asc::lba_out_of_range))))
    ...
    disk->SetBlockCount(1);
    EXPECT_CALL(*controller, Status);
    disk->Dispatch(scsi_command::cmd_read16);
    EXPECT_EQ(status::good, controller->GetStatus());
...
TEST(DiskTest, Write6)
{
    ...
    EXPECT_THAT([&]
        {
            d->Dispatch(scsi_command::cmd_write6)
            ;
        }, Throws<scsi_exception>(AllOf(
...
TEST(DiskTest, Write10)
{
    ...
    EXPECT_THAT([&] {d->Dispatch(scsi_command::cmd_write10);}, Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::illegal_request),
                Property(&scsi_exception::get_asc, asc::lba_out_of_range))))
    ...
    disk->SetBlockCount(1);
    EXPECT_CALL(*controller, Status);
    disk->Dispatch(scsi_command::cmd_write10);
    EXPECT_EQ(status::good, controller->GetStatus());
...
TEST(DiskTest, Write16)
{
    ...
    EXPECT_THAT([&] {d->Dispatch(scsi_command::cmd_write16);}, Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::illegal_request),
                Property(&scsi_exception::get_asc, asc::lba_out_of_range))))
    ...
    disk->SetBlockCount(1);
    EXPECT_CALL(*controller, Status);
    disk->Dispatch(scsi_command::cmd_write16);
    EXPECT_EQ(status::good, controller->GetStatus());
...
TEST(DiskTest, Verify10)
{
    ...
    EXPECT_THAT([&] {d->Dispatch(scsi_command::cmd_verify10);}, Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::illegal_request),
                Property(&scsi_exception::get_asc, asc::lba_out_of_range))))
    ...
    disk->SetBlockCount(1);
    EXPECT_CALL(*controller, Status);
    disk->Dispatch(scsi_command::cmd_verify10);
    EXPECT_EQ(status::good, controller->GetStatus());
...
TEST(DiskTest, Verify16)
{
    ...
    EXPECT_THAT([&] {d->Dispatch(scsi_command::cmd_verify16);}, Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::illegal_request),
                Property(&scsi_exception::get_asc, asc::lba_out_of_range))))
    ...
    disk->SetBlockCount(1);
    EXPECT_CALL(*controller, Status);
    disk->Dispatch(scsi_command::cmd_verify16);
    EXPECT_EQ(status::good, controller->GetStatus());
...
TEST(DiskTest, ReadLong10)
{
    ...
    EXPECT_CALL(*controller, Status);
    disk->Dispatch(scsi_command::cmd_read_long10);
    EXPECT_EQ(status::good, controller->GetStatus());

    controller->SetCmdByte(2, 1);
    EXPECT_THAT([&] {d->Dispatch(scsi_command::cmd_read_long10);}, Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::illegal_request),
                Property(&scsi_exception::get_asc, asc::lba_out_of_range))))
...
TEST(DiskTest, WriteLong10)
{
    ...
    EXPECT_CALL(*controller, Status);
    disk->Dispatch(scsi_command::cmd_write_long10);
    EXPECT_EQ(status::good, controller->GetStatus());

    controller->SetCmdByte(2, 1);
    EXPECT_THAT([&] {d->Dispatch(scsi_command::cmd_write_long10);}, Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::illegal_request),
                Property(&scsi_exception::get_asc, asc::lba_out_of_range))))
...
TEST(DiskTest, WriteLong16)
{
    ...
    controller->SetCmdByte(2, 1);
    EXPECT_THAT([&] {d->Dispatch(scsi_command::cmd_write_long16);}, Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::illegal_request),
                Property(&scsi_exception::get_asc, asc::lba_out_of_range))))
    ...
    EXPECT_CALL(*controller, Status);
    disk->Dispatch(scsi_command::cmd_write_long16);
    EXPECT_EQ(status::good, controller->GetStatus());
...
TEST(DiskTest, StartStopUnit)
{
    ...
    disk->Dispatch(scsi_command::cmd_start_stop);
    EXPECT_EQ(status::good, controller->GetStatus());
    ...
    EXPECT_THAT([&]
        {
            d->Dispatch(scsi_command::cmd_start_stop)
            ;
        }, Throws<scsi_exception>(AllOf(
            Property(&scsi_exception::get_sense_key, sense_key::illegal_request),
...
TEST(DiskTest, PreventAllowMediumRemoval)
{
    ...
    EXPECT_THAT([&] {d->Dispatch(scsi_command::cmd_prevent_allow_medium_removal);}, Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::not_ready),
                Property(&scsi_exception::get_asc, asc::medium_not_present))))
    ...
    EXPECT_CALL(*controller, Status);
    disk->Dispatch(scsi_command::cmd_prevent_allow_medium_removal);
    EXPECT_EQ(status::good, controller->GetStatus());
...
TEST(DiskTest, ModeSense6)
{
    ...
    disk->Dispatch(scsi_command::cmd_mode_sense6);
...
TEST(DiskTest, ModeSense10)
{
    ...
    disk->Dispatch(scsi_command::cmd_mode_sense10);
...
TEST(DiskTest, SynchronizeCache)
{
    ...
    EXPECT_CALL(*disk, FlushCache);
    EXPECT_CALL(*controller, Status);
    disk->Dispatch(scsi_command::cmd_synchronize_cache10);
    EXPECT_EQ(status::good, controller->GetStatus());

    EXPECT_CALL(*disk, FlushCache);
    EXPECT_CALL(*controller, Status);
    disk->Dispatch(scsi_command::cmd_synchronize_cache16);
    EXPECT_EQ(status::good, controller->GetStatus());
}
...
TEST(DiskTest, ReadDefectData)
{
    ...
    EXPECT_CALL(*controller, DataIn);
    disk->Dispatch(scsi_command::cmd_read_defect_data10);
    EXPECT_EQ(status::good, controller->GetStatus());
}
