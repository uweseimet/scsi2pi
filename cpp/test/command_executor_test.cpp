...
TEST(CommandExecutorTest, Attach)
{
    ...
    EXPECT_FALSE(executor.ProcessDeviceCmd(context, definition, false)) << "Unknown device type not rejected";
    ...
    EXPECT_FALSE(executor.ProcessDeviceCmd(context, definition, false))
        << "Drive with invalid sector size not rejected";

    definition.set_block_size(512);
    EXPECT_FALSE(executor.ProcessDeviceCmd(context, definition, false)) << "Drive without image file not rejected";

    SetParam(definition, "file", "/non_existing_file");
    EXPECT_FALSE(executor.ProcessDeviceCmd(context, definition, false))
        << "Drive with non-existing image file not rejected";

    path filename = CreateTempFile(1);
    SetParam(definition, "file", filename.string());
    EXPECT_FALSE(executor.ProcessDeviceCmd(context, definition, false)) << "Too small image file not rejected";
