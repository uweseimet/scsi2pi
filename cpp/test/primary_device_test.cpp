...
TEST(PrimaryDeviceTest, TestUnitReady)
{
    ...
    EXPECT_CALL(*controller, Status);
    device->Dispatch(scsi_command::cmd_test_unit_ready);
...
TEST(PrimaryDeviceTest, Inquiry)
{
    ...
    EXPECT_CALL(*controller, DataIn).Times(0);
    EXPECT_THAT([&] {d->Dispatch(scsi_command::cmd_inquiry);}, Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::illegal_request),
                Property(&scsi_exception::get_asc, asc::invalid_field_in_cdb)))) << "EVPD bit is not supported";

    controller->SetCmdByte(2, 0x01);
    EXPECT_CALL(*controller, DataIn).Times(0);
    EXPECT_THAT([&d] {d->Dispatch(scsi_command::cmd_inquiry);}, Throws<scsi_exception>(AllOf(
                Property(&scsi_exception::get_sense_key, sense_key::illegal_request),
                Property(&scsi_exception::get_asc, asc::invalid_field_in_cdb)))) << "PAGE CODE field is not supported";
