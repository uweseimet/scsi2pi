...
void Controller::Send()
{
    ...
    if (const auto length = GetCurrentLength(); length) {
        ...
        if (const int l = bus.TargetSendHandShake(span(GetBuffer()).subspan(GetOffset(), length),
...
void Controller::Receive()
{
    ...
    if (const auto curr_length = GetCurrentLength(); curr_length) {
        ...
        if (const int l = bus.TargetReceiveHandShake(span(GetBuffer()).subspan(GetOffset(), curr_length)); l
            != curr_length) {
