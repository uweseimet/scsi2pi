    ...
    for (const auto& [key, value] : PropertyHandler::GetInstance().GetProperties()) {
        ...
        if (page_data.empty()) {
            ...
            if (page_code && (page_code != to_integer<int>(page_data[0] & byte { 0x3f }))) {
                warn("Ignored mode page definition with inconsistent page code {}: {}", page_code,
                    to_underlying(page_data[0]));
                continue;
            }

            if (page_code && static_cast<byte>(page_data.size() - 2) != page_data[1]) {
                warn("Ignored mode page definition with wrong page size {}: {}", page_code,
                    to_underlying(page_data[1]));
                continue;
            }
