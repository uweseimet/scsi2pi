...
void DaynaPort::Read6()
{
    ...
    const int length = Read(GetController()->GetCdb(), GetController()->GetBuffer(), record);
...
{
    GetController()->SetLength(RetrieveStats(GetController()->GetCdb(), GetController()->GetBuffer()));
