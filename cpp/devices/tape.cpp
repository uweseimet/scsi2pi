...
void Tape::Read(bool read_16)
{
    ...
    if (expl && !Locate(true)) {
        ...
        throw ScsiException(NO_SENSE, LOCATE_OPERATION_FAILURE);
    }
...
int Tape::ReadData(data_in_t buf)
{
    ...
    LogTrace("Reading {} data byte(s) from position {}, record length is {}", length, tape_position, record_length);
...
int Tape::WriteData(cdb_t, data_out_t buf, int chunk_size)
{
    ...
    LogTrace("Writing {} data byte(s) to position {}, record length is {}", length, tape_position, Pad(record_length));
...
void Tape::Open()
{
    ...
    if (const int append = ParseAsUnsignedInt(GetParam(APPEND)); append == -1) {
        throw ParserException("Invalid maximum file size: '{}'", GetParam(APPEND));
    }
    else {
    ...
    if (max_file_size && max_file_size < GetBlockSize()) {
        throw IoException("Maximum file size {} is smaller than block size {}", max_file_size, GetBlockSize());
    }
...
void Tape::FormatMedium()
{
    ...
    if (tape_position) {
        throw ScsiException(ILLEGAL_REQUEST, SEQUENTIAL_POSITIONING_ERROR);
    }
...
void Tape::WriteMetaData(Tape::ObjectType object_type, uint32_t size)
{
    ...
    if (file_size >= tape_position + META_DATA_SIZE) {
        WriteSimhMetaData(SimhClass::PRIVATE_MARKER,
            (to_underlying(ObjectType::END_OF_DATA) << 24) | PRIVATE_MARKER_MAGIC);
    }
...
{
    LogTrace("Searching for object type {} with count {} at position {}", to_underlying(type_to_find), requested_count,
        tape_position);
    ...
    while (true) {
        ...
        LogTrace("Found object type {}, length {}, moved over {} object(s)", to_underlying(type_found), length,
            actual_count);
...
void Tape::RaiseBeginningOfPartition(int32_t info)
{
    ...
    SetInformation(info);
    SetEom(BEGINNING_OF_PARTITION_MEDIUM_DETECTED);
...
{
    SetInformation(info);
    SetEom(END_OF_PARTITION_MEDIUM_DETECTED);

    throw ScsiException(MEDIUM_ERROR, NO_ADDITIONAL_SENSE_INFORMATION);
}
...
void Tape::RaiseEndOfData(ObjectType object_type, int32_t info)
{
    ...
    LogTrace("Encountered end-of-data at position {} while spacing over object type {}", tape_position,
        to_underlying(object_type));

    SetInformation(info);

    throw ScsiException(BLANK_CHECK, NO_ADDITIONAL_SENSE_INFORMATION);
}

void Tape::RaiseFilemark(int32_t info, bool reverse, bool read)
{
    LogTrace("Encountered filemark at position {} while spacing over blocks",
    ...
    throw ScsiException(NO_SENSE, NO_ADDITIONAL_SENSE_INFORMATION);
}
...
{
    LogError("Trailing record length {} at position {} does not match leading length {}", meta_data.value,
        tape_position, record_length);
...
bool Tape::ReadNextMetaData(SimhMetaData &meta_data, bool reverse)
{
    ...
    LogTrace("Read SIMH meta data with class {:1X}, value ${:07x} at position {}", to_underlying(meta_data.cls),
        meta_data.value, reverse ? tape_position : tape_position - META_DATA_SIZE);
...
uint32_t Tape::GetByteCount()
{
    ...
    LogTrace("Current position: {}, requested byte count: {}", tape_position, count);
...
void Tape::Erase()
{
    ...
    const auto gap = to_underlying(SimhMarker::ERASE_GAP);
    ...
    while (remaining >= 4) {
        ...
        file.write(to_const_char_ptr(buf), chunk);
        CheckForWriteError();
...
pair<Tape::ObjectType, int> Tape::ReadSimhMetaData(SimhMetaData &meta_data, int32_t count, bool reverse)
{
    while (ReadNextMetaData(meta_data, reverse)) {
        ...
        case SimhClass::RESERVERD_MARKER:
            if (meta_data.value == to_underlying(SimhMarker::END_OF_MEDIUM)) {
                RaiseEndOfPartition(count);
            }
            LogTrace(
                fmt::runtime(
                    meta_data.value == to_underlying(SimhMarker::ERASE_GAP) ?
        ...
        case SimhClass::PRIVATE_MARKER:
            if ((meta_data.value & 0x00ffffff) == PRIVATE_MARKER_MAGIC) {
                LogTrace("Found SCSI2Pi private marker for object type {} at position {}",
                    (meta_data.value >> 24) & 0x0f, tape_position - META_DATA_SIZE);
                return {static_cast<ObjectType>((meta_data.value >> 24) & 0x0f), 0};
            }
            LogTrace("Skipping unknown SIMH private marker, value ${:07x} at position {}",
                static_cast<int>(meta_data.value), tape_position - META_DATA_SIZE);
            break;

        default:
            LogTrace("Skipping unknown SIMH class {:1X} at position {}", to_underlying(meta_data.cls),
                tape_position - META_DATA_SIZE);
            if (!reverse && IsRecord(meta_data)) {
                tape_position += Pad(meta_data.value) + META_DATA_SIZE;
            }
...
{
    LogTrace("Writing SIMH meta data with class {:1X}, value ${:07x} to position {}", to_underlying(cls), value,
        tape_position);

    CheckForOverflow(tape_position + META_DATA_SIZE);

    file.write(to_const_char_ptr(ToLittleEndian( { cls, value })), META_DATA_SIZE);
    CheckForWriteError();
...
uint32_t Tape::CheckBlockLength()
{
    if (record_length != byte_count) {
        ...
        if (fixed && byte_count % record_length) {
            ...
            GetController()->SetStatus(CHECK_CONDITION);
...
void Tape::CheckForOverflow(int64_t length)
{
    if (tape_position + length > max_file_size) {
        ++write_error_count;
        throw ScsiException(VOLUME_OVERFLOW);
    }
}
