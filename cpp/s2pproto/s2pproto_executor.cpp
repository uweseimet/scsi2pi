...
string S2pProtoExecutor::Execute(const string &filename, protobuf_format input_format, PbResult &result)
{
    ...
    if (initiator_executor->Execute(scsi_command::execute_operation, cdb, buffer, length, 3, true)) {
    ...
    if (initiator_executor->Execute(scsi_command::receive_operation_results, cdb, buffer, buffer.size(), 3, true)) {
