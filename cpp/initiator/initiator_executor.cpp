...
int InitiatorExecutor::Execute(span<uint8_t> cdb, span<uint8_t> buffer, int length, int timeout)
{
    ...
    if (const int count = CommandMetaData::Instance().GetCommandBytesCount(cmd); count
        ...
        logger.warn("CDB has {0} byte(s), command {1} requires {2} bytes", cdb.size(), command_name, count);
    }

    logger.debug(CommandMetaData::Instance().LogCdb(cdb, "Initiator"));
...
bool InitiatorExecutor::Dispatch(span<uint8_t> cdb, span<uint8_t> buffer, int &length)
{
    ...
    logger.trace("Current phase is {}", Bus::GetPhaseName(phase));
    ...
    default:
        logger.warn("Ignoring {} phase", Bus::GetPhaseName(phase));
        return false;
    }
...
{
    logger.trace("Arbitration with initiator ID {}", initiator_id);

    if (!WaitForFree()) {
        logger.trace("Bus is not free");
        return false;
    }
    ...
    if (bus.GetDAT() > (1 << initiator_id)) {
        logger.trace("Lost arbitration, winning initiator ID is {}", bus.GetDAT() - (1 << initiator_id));
        return false;
    }
...
{
    logger.trace("Selection of target {0} with initiator ID {1}", target_id, initiator_id);
...
void InitiatorExecutor::Command(span<uint8_t> cdb)
{
    ...
    if (static_cast<int>(cdb.size()) < sent_count) {
        if (const string_view &command_name = CommandMetaData::Instance().GetCommandName(cmd); !command_name.empty()) {
            logger.error("Command {} failed", command_name);
        }
        else {
            logger.error("Command ${:02x} failed", static_cast<int>(cmd));
        }
    }
...
void InitiatorExecutor::Status()
{
    ...
    if (bus.ReceiveHandShake(buf.data(), static_cast<int>(buf.size())) != static_cast<int>(buf.size())) {
        logger.error("STATUS phase failed");
    }
    else {
        status = buf[0];
    }
...
void InitiatorExecutor::DataIn(data_in_t buf, int &length)
{
    ...
    logger.trace("Initiator is receiving up to {0} byte(s) in DATA IN phase", length);
...
void InitiatorExecutor::DataOut(data_out_t buf, int &length)
{
    ...
    logger.debug(fmt::format("Initiator is sending {0} byte(s):\n{1}", length, FormatBytes(buf, length, 128)));

    byte_count = bus.SendHandShake(buf.data(), length);
    if (byte_count != length) {
        logger.error("Initiator sent {0} byte(s) in DATA OUT phase, expected size was {1} byte(s)", byte_count, length);
...
void InitiatorExecutor::MsgIn()
{
    ...
    switch (msg) {
    case -1:
        logger.error("MESSAGE IN phase failed");
        break;
    ...
    default:
        logger.trace("Device did not report command completion, rejecting unsupported message ${:02x}", msg);
...
void InitiatorExecutor::MsgOut()
{
    ...
    if (bus.SendHandShake(buf.data(), buf.size()) != buf.size()) {
        logger.error("MESSAGE OUT phase for {} message failed",
...
void InitiatorExecutor::LogStatus() const
{
    if (status) {
        if (const auto &it = STATUS_MAPPING.find(static_cast<status_code>(status)); it != STATUS_MAPPING.end()) {
            logger.warn("Device reported {0} (status code ${1:02x})", it->second, status);
        }
        else if (status != 0xff) {
            logger.warn("Device reported an unknown status (status code ${:02x})", status);
        }
        else {
